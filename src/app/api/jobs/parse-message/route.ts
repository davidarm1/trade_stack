...
- materials: an array of itemised billable lines whenever the message describes more than one distinguishable task, service, or product — this is about the WORK, not the formatting of the message and not whether prices happen to be given per item. Real client messages are messy: a dense run-on paragraph, a WhatsApp export with no punctuation, or one long email sentence joining things with "and"/"plus"/"also" are JUST AS ITEMISABLE as a numbered list or bullet points — do not require visual separators (numbers, bullets, line breaks) before treating something as a separate item; read for meaning instead. For example "can you sort the hosting, look at our email, and the job platform's playing up too" names three separate items with zero list formatting. If a WhatsApp-style paste includes timestamps or sender names, ignore them as noise and read only the substance. When genuinely unsure whether something is one item or several, prefer breaking it down further — the office can always merge rows back together on the form, but can't un-bundle a lump sum. Each item is {"description": string, "quantity": number (default 1), "unit_price": number or null}. Price each item in this order of preference: (1) an explicit price stated for that specific item in the message, (2) a matching rate from the tenant pricing guide below (e.g. the guide says "Drain clearing £150/hr" and this item is drain clearing), (3) otherwise unit_price: null — never invent a figure with no basis in the message or the guide, that's the office's job to fill in. Only return an empty array (and use labour_charge instead) if the message genuinely describes ONE single indivisible task, like "unblock the kitchen sink" — never populate both materials and labour_charge for the same work, that would double the total.
