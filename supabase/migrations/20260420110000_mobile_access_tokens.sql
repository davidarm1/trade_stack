-- Mobile access tokens generated by owner/office per user.
-- These are tenant-scoped and can be revoked at any time.
...
COMMENT ON TABLE public.mobile_access_tokens IS
  'One-time/revocable mobile access tokens generated by office/owner users.';
COMMENT ON COLUMN public.mobile_access_tokens.token_hash IS
  'SHA-256 hash of plaintext token shown once at generation time.';
COMMENT ON COLUMN public.mobile_access_tokens.token_hint IS
  'Display-only hint (last 4 chars) so staff can identify a token without storing plaintext.';
