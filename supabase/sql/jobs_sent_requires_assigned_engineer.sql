...
-- if sent_to_engineer_at is set, assigned_engineer_membership_id must be set.
--
...
WITH latest_completion_engineer AS (
  SELECT DISTINCT ON (jc.job_id)
    jc.job_id,
    jc.tenant_id,
    jc.engineer_membership_id
  FROM public.job_completions jc
  WHERE jc.engineer_membership_id IS NOT NULL
  ORDER BY jc.job_id, jc.submitted_at DESC NULLS LAST, jc.id DESC
)
UPDATE public.jobs j
SET
  assigned_engineer_membership_id = lce.engineer_membership_id,
  updated_at = NOW()
...
WHERE j.id = lce.job_id
  ...
  AND j.assigned_engineer_membership_id IS NULL;
...
ALTER TABLE public.jobs
  ...
  CHECK (sent_to_engineer_at IS NULL OR assigned_engineer_membership_id IS NOT NULL);
